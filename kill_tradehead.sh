...
# Log file for today
LOG_FILE="$LOG_DIR/kill_tradehead_$(date +\%Y\%m\%d).log"
...
echo "$(date) [UTC]: Attempting graceful shutdown of tradehead application" >> "$LOG_FILE"
...
if screen -ls | grep -q "tradehead"; then
    ...
    screen -S tradehead -X stuff '^C'

    echo "$(date) [UTC]: Waiting 10 minutes for tradehead to safely unwind positions..." >> "$LOG_FILE"
    ...
    if screen -ls | grep -q "tradehead"; then
        ...
        if screen -ls | grep -q "tradehead"; then
            ...
            echo "ERROR: Failed to terminate tradehead screen session" >> "$LOG_FILE"
        else
