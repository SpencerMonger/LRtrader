...
class CompositeSignalProvider:
    ...
    def _initialize_providers(self):
        ...
        if self._signal_config.get("enable_predictions", True):
            try:
                self._providers["prediction"] = ClickhouseSignalProvider(
                    ticker_configs=list(self._ticker_configs.values()),
                    signal_config=self._signal_config,
                )
                logger.info("Initialized ClickhouseSignalProvider for crossover events")
        ...
        if self._signal_config.get("enable_news_alerts", False):
            try:
                ...
                enable_dynamic_discovery = self._signal_config.get(
                ...
                logger.info(
                    f"Initialized NewsAlertSignalProvider in {mode} mode with {lookback_minutes} minute lookback"
                )
            except Exception as e:
    ...
    def get_provider_status(self) -> Dict[str, dict]:
        ...
        for provider_name, provider in self._providers.items():
            try:
                ...
                provider_status = {
                    "initialized": True,
                    "active_tickers": len(provider.get_all_active_tickers())
                    if hasattr(provider, "get_all_active_tickers")
                    else 0,
...
def create_signal_provider(
    ticker_configs: list[dict], signal_config: dict = None
